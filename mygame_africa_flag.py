...
mydicafrica = {
	"Algeria" : "flags\\africa\\Algeria.png",
	"Madagascar" : "flags\\africa\\Madagascar.png",
	"Somalia" : "flags\\africa\\Somalia.png",
	"Sudan" : "flags\\africa\\Sudan.png",
	"Kenya" : "flags\\africa\\Kenya.png",
	"Ghana" : "flags\\africa\\Ghana.png",
	"Togo" : "flags\\africa\\Togo.png",
	"Zambia" : "flags\\africa\\Zambia.png"
}

def start_flag_game_africa(screen):
	...
	class Image(pygame.sprite.Sprite):
		def __init__(self, path, position):  # Renamed slef to self
			super().__init__()  # Call parent class's constructor
			self.surf = pygame.image.load(path)
			self.surf = pygame.transform.scale(self.surf, (150, 50))  # Use self.surf instead of self.image
			self.rect = self.surf.get_rect(topleft=position)  # Create image for positioning
	...
	africa_map = pygame.image.load('Africa_map.png')
	# Scale the image to fit the desired dimensions, adjust if necessary
	africa_map = pygame.transform.scale(africa_map, (500, 300))
	...
	#half = (len(mydicafrica) // 2) - 1  # finding the half of the dictionary

	# Populate the container list with country and capital squares
	for x in range(len(mydicafrica)):
		container.append(Image(capitals[x], (horizontalCap, verticalCap)))
		verticalCap += inc
	...
	instructions = font.render("Match the country with the flag",True,(0,0,0))
	...
	chosenCountry = []
	chosenCapitol = []
	numPairs = len(mydicafrica)
	# Main game loop
	while gameOn:
		screen.fill((173, 216, 230))  # Clear screen with light blue
		screen.blit(keepTrack.surf, keepTrack.rect)  # Draw score
		screen.blit(africa_map, (screen.get_width() - 500, 250))  # Draw the Europe map on the right side of the screen
		...
		if countryPressed == True and capitolPressed == False:
			...
			if mydicafrica[pickedCountry] == pickedCapitol:
		...
		if len(chosen) == 2 * numPairs:
			...
			title = font.render(f'Final Score: {score} / {10*len(mydicafrica)}',True,(0,0,0)) # reuse varible
