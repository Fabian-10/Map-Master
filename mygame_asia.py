...
def start_capital_game_asia(screen):
	...
	asia_map = pygame.image.load('Asia_map.png')
	# Scale the image to fit the desired dimensions, adjust if necessary
	asia_map = pygame.transform.scale(asia_map, (500, 300))
	...
	#half = (len(mydicasia) // 2) - 1  # finding the half of the dictionary

	# Populate the container list with country and capital squares
	for x in range(len(mydicasia)):
	...
	chosenCountry = []
	chosenCapitol = []
	numPairs = len(mydicasia)
	# Main game loop
	while gameOn:
		screen.fill((173, 216, 230))  # Clear screen with light blue
		screen.blit(keepTrack.surf, keepTrack.rect)  # Draw score
		screen.blit(asia_map, (screen.get_width() - 500, 250))  # Draw the Europe map on the right side of the screen
		...
		if countryPressed == True and capitolPressed == False:
			...
			if mydicasia[pickedCountry] == pickedCapitol:
		...
		if len(chosen) == 2 * numPairs:
			...
			title = font.render(f'Final Score: {score} / {10*len(mydicasia)}',True,(0,0,0)) # reuse varible
